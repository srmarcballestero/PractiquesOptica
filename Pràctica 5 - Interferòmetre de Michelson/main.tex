...
\begin{abstract}
 \nind L'objectiu d'aquesta pràctica és l'estudi, qualitatiu i quantitatiu, de l'ús i comportament d'un interferòmetre de Michelson, un dels sistemes clàssics d'obtenció d'interferències per divisió de feix. L'instrument consta, essencialment, d'una làmina separadora $\textup{L}$ que reflecteix i transmet a parts iguals la llum que hi incideix, enviant-la a dos miralls perpendiculars entre si, $\textup{E}_1$ i $\textup{E}_2$, per a que finalment retorni a $\textup{L}$ i una part es reflecteixi o transmeti a l'observador. Estudiant la variació del patró interferencial observat segons la diferència de longitud dels braços de l'interferòmetre, s'ha obtingut la longitud d'ona d'un làser díode vermell i una font de llum de sodi, obtenint també la separació de la longitud d'ona del doblet de sodi. Tots els valors obtinguts experimentalment han resultat compatibles amb els esperats. Finalment, s'ha analitzat el comportament del sistema quan se l'il·lumina amb llum blanca en condició d'equidistància, obtenint també un patró interferencial destacable.
\end{abstract}

\input{Figures/Interferometre.tex}

\section{Patró interferomètric} \label{sec:patroInt}
...
Així doncs, la longitud d'ona de la font de sodi s'ha estimat com la mitjana de les dues mesures realitzades, calculada segons \eqref{est:mitjana}, obtenint que $\lambda_\textup{Na,av} = \SI{570 \pm 40}{\nano\metre}$, valor que es compatible amb les longituds d'ona del doblet del sodi\footnote{\textsc{Font:} NIST. \textit{Persistent Lines of Neutral Sodium (Na I)}. National Institute of Standards and Technology - NIST. \url{https://physics.nist.gov/PhysRefData/Handbook/Tables/sodiumtable3.htm}.} $\lambda_\textup{Na} = \SI{589.0}{\nano\metre}$ i $\lambda_\textup{Na}' = \SI{589.6}{\nano\metre}$ dins del marge d'incertesa corresponent.
...
\begin{itemize} [label = --]
 \item \textbf{Mesura 1}. S'ha obtingut que en $D_1 = \SI{7.395(5)}{\milli\metre}$ i $D_2 = \SI{6.025(5)}{\milli\metre}$ hi ha mínims de contrast consecutius, d'on, aplicant \eqref{eq:Dlambda} es té $\Delta \lambda_{\textup{Na},1} = \SI{0.59(8)}{\nano\metre}$.
...
Finalment, la separació del doblet del sodi s'ha estimat com la mitjana entre els dos valors, calculada segons \eqref{est:mitjana}, obtenint que $\Delta \lambda_\textup{Na,av} = \SI{0.59(11)}{\nano\metre}$, que és compatible amb la provinent dels valors de la bibliografia $\Delta \lambda_\textup{Na} = \SI{0.6}{\nano\metre}$, ja que
\begin{equation*}
 \SI{0.01}{\nano\metre} = \left | \Delta\lambda_\textup{Na,av} - \Delta\lambda_\textup{Na} \right | \leq \delta\left(\Delta\lambda_\textup{Na,av}\right) = \SI{0.11}{\nano\metre}
\end{equation*}
...
Es diu que l'interferòmetre està en el \emph{punt d'equidistància} quan la diferència entre la longitud dels braços $d$ és nul·la. En aquesta situació, segons \eqref{eq:Delta} la diferència de camí òptic entre els rajos que interaccionen és nul·la per qualsevol angle $\theta$, i per tant, s'espera que en la pantalla d'observació el patró interferencial sigui completament destructiu, atès que el feix provinent del mirall $\textup{E}_2$ pateix un desfasament de $\pi$ en la reflexió.

Tot il·lumianant el sistema amb una llum de mercuri i una font de llum blanca, s'ha regulat la posició del mirall $\textup{E}_1$ fins observar anells cada cop més separats i deformats. Finalment, s'arriba a la situació en què s'oberven franjes de colors amb saturació màxima al voltant d'una franja completament fosca. Es pot afirmar que el sistema està en el punt d'equidistància.

El fet que s'observi un patró d'interferència es deu al fet que els miralls de l'interferòmetre no són perfectament plans, la qual cosa provoca que la diferència de camí òptic entre les ones que interfereixen no sigui constant en tot el camp visual.

Encara més, si s'apaga la llum de mercuri, se segueixen observant franjes de colors molt saturats degudes a la interferència de la llum blanca, però una petita variació en $d$ les fa desaparèixer ràpidament. Això es deu al fet que els camps que interfereixen són coherents entre sí, al provenir de la mateixa font, però al tractar-se de llum no monocromàtica, la seva longitud de coherència és molt baixa, amb la qual cosa només és possible observar interferències de llum blanca en una regió molt propera al punt d'equidistància.

\input{Figures/LlumBlanca.tex}
