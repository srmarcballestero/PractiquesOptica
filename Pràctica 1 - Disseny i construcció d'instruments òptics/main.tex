...
\end{equation}
i obtenim que val $\Gamma' = -5.0 \pm 0.3$.
...
\end{equation}
on $\diameter_\textup{DC}$ és el diametre del diafragma de camp, que hem près com $\diameter_\textup{DC} = \SI{2.5 \pm 0.1}{\centi\metre}$. Per tant, tenim que el camp d'observació val $2\omega = \SI{0.048\pm0.003}{\radian}$.
...
Finalment, canviem l'ocular per un de potència $\tilde{P}_\textup{ocu} = \SI{20.0 \pm 0.3}{\dioptre}$ i diàmetre $\tilde{\diameter}_\textup{ocu} = \SI{5.0 \pm 0.1}{\centi\metre}$. Fent ús de les fórmules de la \eqref{eq:focals} a la \eqref{eq:augments}, obtenim que en aquest cas, l'emergència de la pupil·la de sortida és $\tilde{e} = \SI{5.50 \pm 0.10}{\centi\metre}$, l'augment $\tilde{\Gamma '} = -10.4 \pm 0.6$. La col·locació d'un diafragma en les mateixes posicions que en el cas anterior exhibeix un comportament anàleg i, en particular, inserint-ne un de $\diameter_\textup{DC} = \SI{2.5 \pm 0.1}{\centi\metre}$ en el pla focal comú es té el mateix camp d'observació que en el cas anterior, vist que l'equació \eqref{eq:semicamp} no depèn de la focal de l'ocular.
...
Un cop dissenyat el sistema d'il·luminació, s'ha col·locat l'objecte a observar a poca distància de la lent col·limadora (per tal d'estalviar espai en en el banc) i s'ha procedit a muntar les lents del microscopi. Com a objectiu s'ha escollit una lent de potència $P_\textup{obj} = \SI{9.5 \pm 0.3}{\dioptre}$ i diàmetre $\diameter_\textup{obj} = \SI{5.5 \pm 0.1}{\centi\metre}$ i per a l'ocular una altra lent de potència $P_\textup{ocu} = \SI{20.5 \pm 0.3}{\dioptre}$ i diàmetre $\diameter_\textup{ocu} = \SI{5.0 \pm 0.1}{\centi\metre}$. Així doncs, aplicant \eqref{eq:focals} obtenim que les focals de les lents són $f'_\textup{obj} = \SI{10.5 \pm 0.3}{\centi\metre}$ i $f'_\textup{ocu} = \SI{4.93 \pm 0.07}{\centi\metre}$.

L'objectiu, que treballa com un projector, s'ha col·locat davant de la diapositiva a una distància superior a la seva focal. Mitjançant una pantalla, s'ha trobat la posició en què s'obté una imatge prou augmentada de la diapositiva a través de l'objectiu, que ha de correspondre a la posició del pla objecte de l'ocular per tal que els raigs surtin paral·lels i l'ull els pugui acomodar. En definitiva, tenim que
\begin{itemize}[label=--]
  \item la distància entre l'objectiu i l'ocular és $d_{\textup{obj} \rightarrow \textup{ocu}} = \SI{82.3 \pm 0.1}{\centi\metre}$,
  \item la distància entre el col·limador i l'objectiu és $d_{\textup{col} \rightarrow \textup{obj}} = \SI{17.3 \pm 0.1}{\centi\metre}$ i
  \item la distància entre l'objectiu i la diapositiva $d_{\textup{obj} \rightarrow \textup{dia}} = \SI{11.3 \pm 0.1}{\centi\metre}$.
\end{itemize}
...
\end{equation}
i val $t = \SI{66.9 \pm 0.3}{\centi\metre}$.
...
\end{equation}
i obtenim que $\beta'_\textup{obj} = -6.37 \pm 0.18$. Així mateix, calculem l'augment angular de l'ocular segons \eqref{eq:augmentull} i obtenim $\Gamma'_\textup{ocu} = 5.1 \pm 0.07$. Conegudes aquestes dades, podem expressar l'augment angular total del sistema com
...
\end{equation}
que resulta $\Gamma' = -32.0 \pm 1.0$.

L'emergència de la pupil·la de sortida vindrà donada per \eqref{eq:pupilasortida} i val $e = \SI{7.25 \pm 0.15}{\centi\metre}$.
...
Finalment, s'ha repetit el muntatge usant un altre ocular de potència $\tilde{P}_\textup{ocu} = \SI{9.5 \pm 0.3}{\dioptre}$ i diàmetre $\tilde{\diameter}_\textup{ocu} = \SI{5.5 \pm 0.1}{\centi\metre}$. La focal calculada segons \eqref{eq:focals} resulta $\tilde{f'}_\textup{ocu} = \SI{10.5 \pm 0.3}{\centi\metre}$. La distància entre l'objectiu i l'ocular és de $\tilde{d}_{\textup{obj} \rightarrow \textup{ocu}} = \SI{90.6 \pm 0.1}{\centi\metre}$.

En aquest cas obtenim que la longitud òptica del tub és $\tilde{t} = \SI{69.6 \pm 0.4}{\centi\metre}$, l'augment lateral de l'objectiu es manté constant, l'augment angular de l'ocular és $\tilde{\Gamma}'_\textup{ocu} = 2.38 \pm 0.07$, l'augment angular total del sistema $\tilde{\Gamma}' = -15.2 \pm 0.6$ i l'emergència de la pupil·la de sortida $e = \SI{21.0 \pm 1.0}{\centi\metre}$.
